...
%
% $Id: StEvent.tex,v 2.75 2003/01/30 18:48:10 ullrich Exp $
%
...
\begin{center}
  ...
  {\LARGE $ $Revision: 2.75 $ $}  \\[5mm] % replaced by cvs with current revision
  {\LARGE $ $Date: 2003/01/30 18:48:10 $ $}  % replaced by cvs with current revision
  \vfill
\end{center}
\cleardoublepage
\end{titlepage}
...
\item[Summary]
\item[Synopsis]
    \verb+#include "StPhmdCluster.h"+\\
    \verb+class StPhmdCluster;+\\
\item[Description]
\item[Related Classes]
\item[Public\\ Constructors]
    \verb+StPhmdCluster(); +\\
\item[Public Member\\ Functions]
    ...
    Module numbers run from 0 to 11.
    
    \verb+int numberOfCells() const;+\\
    Return numbers of cell used in cluster.
...
\item[Public Member\\ Functions]
    ...
    \verb+void deleteCluster(StPhmdCluster* pc);+\\
    Delete cluster pointed to by \texttt{pc}.
...
\item[Public Member\\ Functions]
    ...
    Return detector ID. Can be either \texttt{kPhmdId} or
    for the charged particle veto \texttt{kPhmdCpvId}.
    
    \verb+unsigned int numberOfModules() const;+\\
    Currently always returns 12.
    ...
    Module numbers run from 0-11.
    
    \verb+int moduleHits(unsigned int m);+\\
    Returns number of hits in module \texttt{m}. Module
    numbers run from 0 to 11.
    ...
    \verb+void setModule(StPhmdModule*, unsigned int m);+\\
    Module numbers \texttt{m} runs from 0 to 11.
\end{Entry}
\clearpage
...
\item[Public\\ Constructors]
    \verb+StPhmdHit(); +\\
\item[Public Member\\ Functions]
    \verb+int superModule() const; //! function for supermodule no.+\\
    \verb+int module() ; //! function for module+\\
    \verb+int subDetector() const; //! function for subdetector+\\
    \verb+int row() const; //! function for row+\\
    \verb+int column() const; //! function for col+\\
    \verb+float energy() const; //! function for edep+\\
    \verb+int adc() const; //! function for adc+\\
