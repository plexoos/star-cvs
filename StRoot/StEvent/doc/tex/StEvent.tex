...
%
% $Id: StEvent.tex,v 2.34 2000/05/24 15:43:38 ullrich Exp $
%
...
%
% $Log: StEvent.tex,v $
% Revision 2.34  2000/05/24 15:43:38  ullrich
% More on the StEventMaker (update).
%
% Revision 2.34  2000/05/24 15:43:38  ullrich
% More on the StEventMaker (update).
%
...
\newcommand{\entrylabel}[1]{\mbox{\textbf{{#1}}}\hfil}%
  {\LARGE $ $Revision: 2.34 $ $}  \\[5mm] % replaced by cvs with current revision
  {\LARGE $ $Date: 2000/05/24 15:43:38 $ $}  % replaced by cvs with current revision
...
\index{empty StEvent} \index{Makers and Chains} Sometimes \StEvent\ 
need to be created even if no DST datasets or tables are available.
Since version 2.24 of \texttt{StEventMaker.cxx} this is possible.  In
this case an empty instance of the \texttt{StEvent} class is created,
i.e.~everything
is initialized to zero.\\
Slowly but steadily, more and more ``makers'' will write directly into
\StEvent\ and not use the detour via tables. Any maker who wants to
access \texttt{StEvent} has to be placed behind the
\texttt{StEventMaker} in the chain.

To obtain a pointer to \StEvent\ use:
\begin{verbatim}
...
\item[Public Member\\ Functions]
    ...
    \verb+const StEmcCollection* emcCollection() const;+\\
    ...
    The primary vertices are ordered according to the number of daughters
    (i.e.~primary tracks) they hold. The first in the list is always the
    \verb+void setSsdHitCollection(StSsdHitCollection*);+\\
...
\subsection{StTrack}
    Track quality control flag. Below is a detailed explanation
    provided by Helen Caines.
    \index{track quality flag}\index{iflag}\index{flag() in StTrack}
    
    The most important point is: flag() $>$ 0 is good flag() $<$ 0 is
    bad.
    
    flag() = \texttt{xyy}, where \texttt{x} indicates the detectors
    included in the fit and \texttt{yy} indicates the status of the
    fit. Positive iflag() return values are good fits, negative values
    are bad fits.
    ...
    \item[x=1] TPC only 
    \item[x=2] SVT only 
    \item[x=3] TPC + primary vertex 
    \item[x=4] SVT + primary vertex 
    \item[x=5] SVT+TPC 
    \item[x=6] SVT+TPC+primary vertex 
    \item[x=7] FTPC only 
    \end{description} 
