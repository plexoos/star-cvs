#include "SvttGeo7.h"  
 ...
 {     
       ...
       // ---------------------------------------------------------------------------------------------------     
          ...
          Soup_t soup;     
          //     
          ///@addtogroup SvttGeo7_vars     
          ///@{        
                Int_t ilayer,s,side,ilad,iwaf,i,j,ipos;        
                //        
                /// Int_t ilayer,s,side,ilad,iwaf,i,j,ipos        
          ///@}     
          ///@addtogroup SvttGeo7_vars     
          ...
          ///@addtogroup SvttGeo7_vars     
          ///@{        
                Float_t xdisp,ydisp,adisp,cs;        
                //        
                /// Float_t xdisp,ydisp,adisp,cs        
          ///@}     
          ///@addtogroup SvttGeo7_vars     
          ///@{        
                Int_t i_phi,checkrez,foo,lsub;        
                //        
                /// Int_t i_phi,checkrez,foo,lsub        
          ///@}     
          ///@addtogroup SvttGeo7_vars     
          ///@{        
                Array_t<Float_t> xd(6,16);        
                /// xd(6,16) : array of Float_t        
                Array_t<Float_t> yd(6,16);        
                /// yd(6,16) : array of Float_t        
                Array_t<Float_t> ad(6,16);        
                /// ad(6,16) : array of Float_t        
          ///@}     
       SvttGeo7::SvttGeo7()     
         : AgModule("SvttGeo7","  is extension of the SVTTGEO6: clamshells and distortions ")     
       {        
       }     
          // ---------------------------------------------------------------------------------------------------     
          ...
          {         
                ...
                ///@{           
                      ...
                      /// Loop on ilayer from svtg.nmin to min(6,nint(svtg.nlayer)) step=1           
                      for ( ilayer=svtg.nmin; (1>0)? (ilayer<=min(6,nint(svtg.nlayer))):(ilayer>=min(6,nint(svtg.nlayer))); ilayer+=1 )           
                      {              
                            if ( ilayer<6 )              
                            {                 
          ...
          {         
                ...
                ///@{           
                      ...
                      /// Loop on ipos from 1 to svtl.nladder step=1           
                      for ( ipos=1; (1>0)? (ipos<=svtl.nladder):(ipos>=svtl.nladder); ipos+=1 )           
                      {              
                            xdisp=0.0 ;// xd(ilayer,ipos);              
                            ydisp=0.0 ;// yd(ilayer,ipos);              
                            adisp=ad(ilayer,ipos);              
                            lsub    = mod(ilayer-1,2);              
                            checkrez =  svtl.nladder*2.0-(ipos-1)*2-lsub;              
                            foo =  svtl.nladder;              
                            ...
                            { AgPlacement place = AgPlacement("SLSD","SLYD");                 
                                  /// Add daughter volume SLSD to mother SLYD                 
                                  place.TranslateX(xdisp);                 
                                  /// Translate x = xdisp                 
                                  place.TranslateY(ydisp);                 
                                  /// Translate y = ydisp                 
                                  place.par("only")=AgPlacement::kMany;                 
                                  /// Overlap: agplacement::kmany                 
                                  place.AlphaZ(c0+(360/svtl.nladder)*(ipos-1)+(180/svtl.nladder));                 
                                  /// Rotate: AlphaZ = c0+(360/svtl.nladder)*(ipos-1)+(180/svtl.nladder)                 
                                  /// G3 Reference: thetax = 90                 
                                  /// G3 Reference: phix = 0                 
                                  /// G3 Reference: thetay = 90                 
                                  /// G3 Reference: phiy = 90                 
                                  /// G3 Reference: thetaz = 0                 
                                  /// G3 Reference: phiz = 0                 
                                  _stacker -> Position( AgBlock::Find("SLSD"), place );                 
                            } // end placement of SLSD              
                      }           
                      END_OF_SLYD:           
                      mCurrent = _save;           
                ///@}        
          ...
          {         
                ...
                ///@{           
                      ...
                      { AgAttribute attr = AgAttribute("SLSD");              
                            attr.par("serial")=ilayer*10+ipos;              
                            attr.Inherit( AgBlock::previous() );               
                            _attribute = attr;              
                      }           
                      if ( svtg.nlayer<0 )           
                      ...
                      {              
                            ...
                            {  AgShape shape = AgShape("Tubs");                 
                                  ...
                                  shape.par("rmin")=rmin;                 
                                  shape.par("rmax")=rmax;                 
                                  shape.par("phi1")=-180/svtl.nladder;                 
                                  shape.par("phi2")=180/svtl.nladder;                 
                                  shape.par("dz")=swca.length/2;                 
                                  /// Shape Tubs rmin=rmin rmax=rmax phi1=-180/svtl.nladder phi2=180/svtl.nladder dz=swca.length/2                  
                                  _same_shape &= _stacker->SearchVolume( shape, _attribute );                 
                                  _shape = shape;                 
                                  if (_same_shape) goto END_OF_SLSD;                 
                                  _stacker -> Build(this);                 
          ...
          {         
                ...
                ///@{           
                      ...
                      { AgMaterial &mat = AgMaterial::Get("Senssi");              
                            mat.par("isvol")=1;              
                            _material = mat;              
                      }           
                      ...
                      trapy=swca.senswid/2.0-(swca.senswid-swca.sensgir)/2.0;           
                      _create = AgCreate("STRA");           
                      {              
                            AgShape myshape; // undefined shape              
                            ///Create STRA              
                            Create("STRA");               
                      }           
                      { AgPlacement place = AgPlacement("STRA","SVTD");              
                            /// Add daughter volume STRA to mother SVTD              
                            place.TranslateY(+trapy);              
                            /// Translate y = +trapy              
                            place.AlphaZ(+90);              
                            /// Rotate: AlphaZ = +90              
                            /// G3 Reference: thetax = 90              
                            ...
                            place.Ortho( "YZX" ); // ORT=YZX              
                            /// Axis substitution: XYZ --> YZX              
                            _stacker -> Position( AgBlock::Find("STRA"), place );              
                      } // end placement of STRA           
                      { AgPlacement place = AgPlacement("STRA","SVTD");              
                            /// Add daughter volume STRA to mother SVTD              
                            place.TranslateY(-trapy);              
                            /// Translate y = -trapy              
                      ...
                      ssidx=swca.waferlen/2.0-(swca.waferlen-swca.senslen)/4.0;           
                      ssiry=swca.waferwid/2.0-(swca.waferwid-swca.senswid)/4.0;           
                      _create = AgCreate("SSID");           
                      {              
                            AgShape myshape; // undefined shape              
                            ///Create SSID              
                            Create("SSID");               
                      }           
                      { AgPlacement place = AgPlacement("SSID","SVTD");              
                            /// Add daughter volume SSID to mother SVTD              
                            place.TranslateX(+ssidx);              
                            /// Translate x = +ssidx              
                            _stacker -> Position( AgBlock::Find("SSID"), place );              
                      } // end placement of SSID           
                      { AgPlacement place = AgPlacement("SSID","SVTD");              
                            /// Add daughter volume SSID to mother SVTD              
                            place.TranslateX(-ssidx);              
                            /// Translate x = -ssidx              
                      ...
                      _create = AgCreate("SSIR");           
                      {              
                            AgShape myshape; // undefined shape              
                            ///Create SSIR              
                            Create("SSIR");               
                      }           
                      { AgPlacement place = AgPlacement("SSIR","SVTD");              
                            /// Add daughter volume SSIR to mother SVTD              
                            place.TranslateY(+ssiry);              
                            /// Translate y = +ssiry              
                            _stacker -> Position( AgBlock::Find("SSIR"), place );              
                      } // end placement of SSIR           
                      { AgPlacement place = AgPlacement("SSIR","SVTD");              
                            /// Add daughter volume SSIR to mother SVTD              
                            place.TranslateY(-ssiry);              
                            /// Translate y = -ssiry              
          ...
          // ---------------------------------------------------------------------------------------------------     
          void SSID::Block( AgCreate create )     
          {         
                ///@addtogroup SSID_doc        
                ///@{           
                        AgBlock *_save = mCurrent;           
                      ...
                      { AgMaterial &mat = AgMaterial::Get("Nonsenssi");              
                            mat.par("isvol")=0;              
                            _material = mat;              
                      }           
                      { AgAttribute attr = AgAttribute("SSID");              
                            attr.par("seen")=1;              
                            attr.par("colo")=1;              
                            attr.Inherit( AgBlock::previous() );               
                            _attribute = attr;              
                      ...
                      {  AgShape shape = AgShape("Bbox");              
                            shape     .Inherit( AgBlock::previous() );              
                            create     .SetParameters(shape);              
                            shape.par("dx")=(swca.waferlen-swca.senslen)/4.0;              
                            shape.par("dy")=swca.waferwid/2.0;              
                            /// Shape Bbox dx=(swca.waferlen-swca.senslen)/4.0 dy=swca.waferwid/2.0               
                            _same_shape &= _stacker->SearchVolume( shape, _attribute );              
                            _shape = shape;              
                            if (_same_shape) goto END_OF_SSID;              
                            _stacker -> Build(this);              
                      }           
                      END_OF_SSID:           
                      mCurrent = _save;           
          ...
          // ---------------------------------------------------------------------------------------------------     
          void SSIR::Block( AgCreate create )     
          {         
                ///@addtogroup SSIR_doc        
                ///@{           
                        AgBlock *_save = mCurrent;           
                      ...
                      { AgMaterial &mat = AgMaterial::Get("Nonsenssil");              
                            mat.par("isvol")=0;              
                            _material = mat;              
                      }           
                      { AgAttribute attr = AgAttribute("SSIR");              
                            attr.par("seen")=1;              
                            attr.par("colo")=1;              
                            attr.Inherit( AgBlock::previous() );               
                            _attribute = attr;              
                      ...
                      {  AgShape shape = AgShape("Bbox");              
                            shape     .Inherit( AgBlock::previous() );              
                            create     .SetParameters(shape);              
                            shape.par("dx")=swca.senslen/2.0;              
                            shape.par("dy")=(swca.waferwid-swca.senswid)/4.0;              
                            /// Shape Bbox dx=swca.senslen/2.0 dy=(swca.waferwid-swca.senswid)/4.0               
                            _same_shape &= _stacker->SearchVolume( shape, _attribute );              
                            _shape = shape;              
                            if (_same_shape) goto END_OF_SSIR;              
                            _stacker -> Build(this);              
                      }           
                      END_OF_SSIR:           
                      mCurrent = _save;           
          ...
          // ---------------------------------------------------------------------------------------------------     
          void STRA::Block( AgCreate create )     
          {         
                ///@addtogroup STRA_doc        
                ///@{           
                        AgBlock *_save = mCurrent;           
                      ...
                      { AgMaterial &mat = AgMaterial::Get("Nonsenssili");              
                            mat.par("isvol")=0;              
                            _material = mat;              
                      }           
                      { AgAttribute attr = AgAttribute("STRA");              
                            attr.par("seen")=1;              
                            attr.par("colo")=2;              
                            attr.Inherit( AgBlock::previous() );               
                            _attribute = attr;              
                      ...
                      {  AgShape shape = AgShape("Trd1");              
                            ...
                            create     .SetParameters(shape);              
                            shape.par("dx1")=0.0;              
                            shape.par("dx2")=swca.senswid/2.0;              
                            shape.par("dy")=swca.waferthk/2.0;              
                            shape.par("dz")=(swca.senslen-swca.sensgir)/2.0;              
                            /// Shape Trd1 dx1=0.0 dx2=swca.senswid/2.0 dy=swca.waferthk/2.0 dz=(swca.senslen-swca.sensgir)/2.0               
                            _same_shape &= _stacker->SearchVolume( shape, _attribute );              
                            _shape = shape;              
                            if (_same_shape) goto END_OF_STRA;              
                            _stacker -> Build(this);              
                      }           
                      END_OF_STRA:           
                      mCurrent = _save;           
          ...
          {         
                ...
                ///@{           
                      ...
                      sq=scbp.len*AgPower<Int_t>(2)/(scbp.rmin2-scbp.rmin1)*AgPower<Int_t>(2);           
                      a=pi*(scbp.rmin1*AgPower<Int_t>(2)+scbp.rmin2*AgPower<Int_t>(2))*sqrt(1+sq);           
          ...
          {         
                ...
                ///@{           
                      ...
                      sq=sfep.len*AgPower<Int_t>(2)/(sfep.rmin2-sfep.rmin1)*AgPower<Int_t>(2);           
                      a=pi*(sfep.rmin1*AgPower<Int_t>(2)+sfep.rmin2*AgPower<Int_t>(2))*sqrt(1+sq);           
    ...
    // ----------------------------------------------------------------------- geoctr
       ...
       {        
             ...
             ///@{           
                   ...
                   /// swca . wafcarth = 0.0300; //  wafer carrier thickness             
                   swca . wafergap = 0.0; //  inter-wafer gap - there is a 50 micron uncertainty            
                   /// swca . wafergap = 0.0; //  inter-wafer gap - there is a 50 micron uncertainty            
                   swca . drift = 1; //  drift direction            
                   /// swca . drift = 1; //  drift direction            
                   swca . strutlen = 1.0; //  len (z) of strut between waf car. rails (approx)            
                   /// swca . strutlen = 1.0; //  len (z) of strut between waf car. rails (approx)            
                   swca . senswid = 6.000; //  Define the sensitive square width  (new)            
                   /// swca . senswid = 6.000; //  Define the sensitive square width  (new)            
                   swca . senslen = 6.000; //  Define the sensitive square length (new)            
                   /// swca . senslen = 6.000; //  Define the sensitive square length (new)            
                   swca . sensgir = 5.775; //  Girth (for lack of a better word)            
                   /// swca . sensgir = 5.775; //  Girth (for lack of a better word)            
                   //           
                   swca.fill();           
             ///@}        
