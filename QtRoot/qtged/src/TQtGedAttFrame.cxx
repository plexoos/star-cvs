// @(#)root/ged:$Name:  $:$Id: TQtGedAttFrame.cxx,v 1.4.2.1 2016/04/27 16:45:43 jeromel Exp $
...
TQtGedAttInterface::TQtGedAttInterface(QMainWindow  *mainWidget, const QString &label, TCanvas *canvas,
                          ...
                          : QDockWindow(QDockWindow::InDock,mainWidget)
...
//  MakeTitle((const char *)label);
   }
   fPanelBox    = new QHBox(this);
   fPanelBox->setMaximumSize(QSize(width,32767));
   fPanelBox->setSpacing(2);
   fPanelBox->setMargin(2);
   boxLayout()->addWidget(fPanelBox);
...
{   
   ...
   if (p && p[0]) {
      //     setLabel(p);
      QString tooltip;
      tooltip = QString("%1 class editor panel").arg(p);
      QToolTip::add(this,tooltip);
      ...
      if (!parent) {
         ...
         box     = new QHBox(parent);
      }
      new QLabel(p,box);box->setSpacing(0);
      QFrame *line1 = new QFrame( box, "line1" );
