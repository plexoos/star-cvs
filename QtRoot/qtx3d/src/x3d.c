/* @(#)root/x3d:$Name:  $:$Id: x3d.c,v 1.2.4.1 2016/05/23 18:33:11 jeromel Exp $ */
...
{
   if(event == NULL){
      fprintf(stderr, "WARNING: Null event in CheckEvent()!! Display=%p arg =%p\n",display,arg);
      }
...
/* Resize our pix etc... */
         ...
         case ClientMessage:
              if ((Atom)event.xclient.data.l[0] == wm_protocols[0])
                  /* WM_DELETE_WINDOW */
              {
              /*
              ...
              else if ((Atom)event.xclient.data.l[0] == wm_protocols[1])
                  /* WM_SAVE_YOURSELF */
              {
...
/* Try to get rid of a few of the mode change glitches (several exist yet) */
   if (o) {} /*  to suspend the compilation warning */
   if(g->modeChanged){
      XSetPlaneMask(g->dpy, g->gc, AllPlanes);
